...
----------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------
local preparingMessageStyle = function(type, isUsingLoad, n)
	--[[
...
local throwError = function(t, s, at, l)
	...
	local tl, d, i =
		preparingMessageStyle("title", true),
		preparingMessageStyle("description", true),
		preparingMessageStyle("index", true)
...
----------------------------------------------------------------------------------------------------
local getMessageStyleColors = function(t, isString)
	--[[
		can be selected in these:
			- error
			- warning
...
----------------------------------------------------------------------------------------------------
local origsin, origcos, origtan = function(n)
		return math.sin(math.rad(n))
	end, function(n)
		return math.cos(math.rad(n))
	end, function(n)
		return math.tan(math.rad(n))
	end
...
local saving,
	...
	function(bn, s)
		...
		if checkRm() then
			...
			if type(g) == "boolean" and g == false then
				return 0
			else
				return 1
			end
